...
function MetricRow({ m }: { m: EndpointMetric }) {
  ...
  return (
    <tr className="border-t border-border/60">
      ...
      <td className="py-2 px-3 font-mono text-center text-ink2/65">
  ...
  return (
    <main className="min-h-screen p-6 max-w-5xl mx-auto">
      ...
      <section className="mb-8">
        ...
        <p className="text-xs text-ink2/55 mt-2">
        ...
        <section className="grid md:grid-cols-2 gap-6 mb-8">
          <div className="p-4 rounded-lg bg-panel border border-border">
            ...
            <dl className="text-sm space-y-1 text-gray-300">
              <div><span className="text-ink2/55">อัลกอริทึม:</span> {String(info.methodology.algorithm)}</div>
              <div><span className="text-ink2/55">ฟีเจอร์:</span> {String(info.methodology.features)}</div>
              <div><span className="text-ink2/55">Applicability Domain:</span> {String(info.methodology.applicability_domain)}</div>
              <div><span className="text-ink2/55">Validation:</span> {String(info.methodology.validation)}</div>
            </dl>
            <h4 className="font-medium mt-3 mb-1 text-sm">Confidence 3 ชั้น</h4>
            <ul className="text-xs text-ink2/65 list-disc list-inside space-y-0.5">
      ...
      {info && (
        <p className="text-xs text-ink2/55 pt-4 border-t border-border">{info.disclaimer_th}</p>
      )}
    </main>
  );
}
